...
- [x] **T1.1**: Inicializar projeto Python 3.12 estruturado com `uv` (`pyproject.toml` e `uv.lock`).
- [x] **T1.2**: Fixar a versão exata do `google-adk` na série 2 (`google-adk==2.2.0` ou versão estável superior), juntamente com `fastapi`, `uvicorn`, `pydantic-settings`, `aiosqlite`, `pytest`, `httpx`.
- [x] **T1.3**: Criar `.env.example` contendo `GEMINI_API_KEY` e configurações necessárias, garantindo que `.env` esteja estritamente ignorado no `.gitignore`.
- [x] **T1.4**: Configurar estrutura modular de diretórios (`app/api`, `app/domain`, `app/agents`, `app/scripts`, `tests`).
...
- [x] **T2.1**: Implementar `app/database.py` com suporte a SQLite durável em disco (`aurora.db`), habilitando `PRAGMA journal_mode=WAL`, chaves estrangeiras e o índice exclusivo:
  ```sql
  CREATE UNIQUE INDEX IF NOT EXISTS uq_reservas_area_data_ativa 
  ON reservas(area, data) WHERE status = 'ATIVA';
  ```
- [x] **T2.2**: Implementar `app/domain/services.py` com métodos determinísticos:
...
- [x] **T2.3**: Implementar mecanismo de concorrência atômica da **Garantia 5**:
...
- [x] **T2.4**: Criar script de restauração `app/scripts/restore_data.py`:
...
- [x] **T3.1**: Criar modelo e tabela de confirmações pendentes (`confirmacoes`) rastreando `id`, `session_id`, `apartamento`, `acao`, `detalhes_json`, `status` (`PENDENTE`, `APROVADA`, `NEGADA`).
- [x] **T3.2**: Implementar regra de negócio determinística de necessidade de confirmação:
...
- [x] **T3.3**: Implementar idempotência e blindagem da rota de confirmações:
...
- [x] **T4.1**: Configurar `SessionService` baseado em SQLite no Google ADK (`app/agents/runtime.py`) para persistir o histórico de eventos e o `session.state`.
- [x] **T4.2**: Implementar as ferramentas (*tools*) do ADK com validação estrita de identidade (**Garantia 2**):
...
- [x] **T4.3**: Implementar ferramenta de consulta pontual do regulamento (`app/agents/tools/regulamento_tools.py`):
...
- [x] **T4.4**: Construir Especialista em Reservas (`reservas_specialist`) com as tools de consulta neutra, solicitação e cancelamento próprio.
- [x] **T4.5**: Construir Especialista em Portaria e Visitantes (`visitantes_specialist`) com tool de autorização pendente e consulta de autorizações da unidade.
- [x] **T4.6**: Construir Especialista em Regulamento (`regulamento_specialist`) acoplado à tool de consulta pontual.
- [x] **T4.7**: Construir Agente Principal / Orquestrador (`aurora_orchestrator`):
...
- [x] **T4.8**: Configurar o `App` e o fluxo de retomada (`resume`) do ADK no `Runner` para processamento correto das respostas de confirmação.
...
- [x] **T5.1**: Definir modelos Pydantic rigorosos para requisições e respostas em `app/api/schemas.py`.
- [x] **T5.2**: Implementar rota `POST /sessoes`:
  * Cria a sessão no ADK, grava o apartamento em `state["apartamento"]` e retorna `201 {"session_id": "..."}`.
- [x] **T5.3**: Implementar rota `POST /sessoes/{session_id}/mensagens`:
...
- [x] **T5.4**: Implementar rota `POST /sessoes/{session_id}/confirmacoes`:
...
- [x] **T6.1**: Configurar `pytest` e `httpx.AsyncClient` em `tests/conftest.py`.
- [x] **T6.2**: Testar **Passo 1**: Restauração dos dados iniciais e conferência das rotas de verificação (`101` com `RSV-1377` e `302` com Marina Duarte).
- [x] **T6.3**: Testar **Passos 2 a 5**: Criação de S1 (101); tentativa de espionagem do 302 sem vazamento de `RSV-4821` nem Marina Duarte; tentativa de cancelamento da reserva do 302 frustrada; cancelamento da própria reserva da quadra com sucesso sem confirmação.
- [x] **T6.4**: Testar **Passos 6 a 9**: Reserva da quadra sem confirmação pendente; reserva de salão com taxa gerando confirmação; rejeição sem gravação; aprovação com gravação única; reenvio retornando `409`; confirmação de ID inexistente retornando `409`; sessão inexistente em eventos retornando `404`.
- [x] **T6.5**: Testar **Passos 10 a 12**: Sessão S2 tentando reservar data ocupada pelo 302 sem vazar código nem número 302; autorização de visitante com tentativa de bypass no texto gerando confirmação obrigatória e gravação pós-aprovação; pergunta sobre piscina aos domingos com horário correto sem capítulos extras nos eventos.
- [x] **T6.6**: Testar **Passo 13**: Simulação de reinício do servidor (novo app/runner sobre o mesmo banco SQLite); conferência da contagem de eventos de S1; envio de nova mensagem após reinício; validação de unicidade de códigos de reserva.
- [x] **T6.7**: Testar **Passo 14**: Disputa concorrente simultânea de reservas entre 101 e 201 no salão de festas na mesma data; ambas requisições respondem `200`; soma total de reservas ativas igual a exatamente 1.
- [x] **T6.8**: Testar **Passo 15**: Verificação estática e estrutural de todos os critérios de aceite do repositório.
...
- [x] **T7.1**: Substituir o conteúdo de `README.md` pelo formato final exigido:
...
- [x] **T7.2**: Garantir integridade dos arquivos originais em `dados/`.
- [x] **T7.3**: Git commit e push final para a branch `main` do fork público.
